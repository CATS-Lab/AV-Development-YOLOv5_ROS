...
from yolo5.msg import BoundingBox, BoundingBoxes
...
class Detector:
    def __init__(self):
        ...
        # self.source = rospy.get_param("~source", "~/catkin_ws/src/yolo5/data/images") 
        ...
        if self.compressed_input:
            self.image_sub = rospy.Subscriber(
                input_image_topic, CompressedImage, self.run, queue_size=1
            )
        else:
            self.image_sub = rospy.Subscriber(
                input_image_topic, Image, self.run, queue_size=1
            )
